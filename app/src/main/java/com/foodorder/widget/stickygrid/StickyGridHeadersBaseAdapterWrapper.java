...
package com.foodorder.widget.stickygrid;
...
public class StickyGridHeadersBaseAdapterWrapper extends BaseAdapter {
    ...
    public int getGridViewPositionFromHeaderId(int headerId) {
        int position = 0;
        for (int i = 0; i < mDelegate.getNumHeaders(); i++) {
            if (i == headerId) {
                break;
            }
            position += mDelegate.getCountForHeader(i) + unFilledSpacesInHeaderGroup(i) + mNumColumns;
